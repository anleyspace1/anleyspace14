...
export default function HomePage() {
  ...
  return (
    <div className="w-full lg:max-w-3xl lg:mx-auto space-y-4 lg:space-y-6">
      ...
      <Stories />
      <CreatePost onGoLive={() => navigate('/live?host=true')} />
...
function CreatePost({ onGoLive }: { onGoLive: () => void }) {
  const navigate = useNavigate();
...
function CreatePostModal({ onClose }: { onClose: () => void }) {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      onClose();
      // Refresh posts if needed (the parent component should handle this via real-time or re-fetch)
...
function Feed({ category }: { category?: string | null }) {
  const { user } = useAuth();
  ...
  const fetchPosts = async () => {
    try {
      ...
      // Group by post_id
      const commentCounts = commentData.reduce((acc: any, curr: any) => {
      ...
      const formattedPosts = data.map(post => ({
        ...
        user: { 
          ...
          avatar: post.profiles?.avatar_url || `https://picsum.photos/seed/${post.user_id}/100/100`, 
...
function PostItem({ post, onDelete }: { post: any; onDelete: () => void; key?: React.Key }) {
  ...
  const fetchComments = async () => {
    ...
    try {
      ...
      const formattedComments = data.map(c => ({
        ...
        text: c.content,
        avatar: c.profiles?.avatar_url || `https://picsum.photos/seed/${c.user_id}/100/100`,
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newCommentObj = {
        id: data.id,
        user: data.profiles?.username || 'me',
        text: data.content,
        avatar: data.profiles?.avatar_url || `https://picsum.photos/seed/${data.user_id}/100/100`,
        time: 'now'
      };
      ...
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] text-gray-400">{post.user.time}</span>
          </div>
        </div>
        <PostMenu 
          isMe={post.user.isMe} 
